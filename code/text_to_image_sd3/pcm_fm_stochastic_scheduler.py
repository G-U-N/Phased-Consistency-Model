...
@dataclass
class PCMFMStochasticSchedulerOutput(BaseOutput):
    prev_sample: torch.FloatTensor


class PCMFMStochasticScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
    ...
    ) -> Union[PCMFMStochasticSchedulerOutput, Tuple]:
        """
        ...
        sigma_prev = self.sigmas_[self.step_index + 1]
        prev_sample = (1 - sigma_prev) * denoised + sigma_prev * torch.randn_like(
            denoised
        )
        ...
        return PCMFMStochasticSchedulerOutput(prev_sample=prev_sample)
